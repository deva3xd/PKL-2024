        ...
        <div class="w-1/2 flex justify-center items-center bg-white">
            <div class="text-center text-black w-7/12">
                ...
                <form method="POST" action="{{ route('postLogin') }}">
                    @csrf
                    <label class="mt-5 h-14 rounded-xl input input-bordered flex items-center gap-2" style="background-color: #f1f1f1">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 opacity-70"><path d="M8 8a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM12.735 14c.618 0 1.093-.561.872-1.139a6.002 6.002 0 0 0-11.215 0c-.22.578.254 1.139.872 1.139h9.47Z" /></svg>
                        <input type="text" class="grow" placeholder="Username" name="username" required />
                    </label>
                    <label class="mt-4 h-14 rounded-xl input input-bordered flex items-center gap-2" style="background-color: #f1f1f1">
                    ...
                    <button type="submit" class="btn border-none mt-5 p-4 rounded-xl text-white w-full bg-blue-700 hover:bg-blue-800 font-bold text-1xl">LOG IN</button>
                </form>
                <p class="mt-2">Belum punya akun? <a href="{{ route('register') }}" class="text-blue-700">Buat</a> Sekarang</p>
            </div>
        </div>
    </div>
@endsection